...
class adict(dict):
  """
  Convenience data structure to use
  attributes to read/write dictionary content
  """
...
def conv2d(input_, output_dim, k_h, k_w, name="conv2d"):
  ...
  # Input Dim:
  # [batch_size x max_word_length(height) x embed_size(width) x kernel_feature_size]
  # to compute 2-D convolution
...
def inference_graph(char_vocab_size, word_vocab_size, char_embed_size=15,
  ...
  with tf.variable_scope('LSTM'):
    ...
    if num_rnn_layers > 1: # stack layers
      cell = tf.contrib.rnn.MultiRNNCell([create_rnn_cell() for _ in range(num_rnn_layers)],
                                         state_is_tuple=True)
    else: # single-cell
...
def loss_graph(logits, batch_size, num_unroll_steps):
  with tf.variable_scope('Loss'):
    ...
    x_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=target_list)
...
def training_graph(loss, learning_rate=1.0, max_grad_norm=5.0):
  ...
  with tf.variable_scope('SGD_Training'): #'SGD_Training'
